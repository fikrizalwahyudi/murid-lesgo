...
export class HomePage {
  ...
  constructor(public network : Network, public localNotif : LocalNotifications, public zapier : Zapier, public Camera : Camera, public File : File, public FilePath : FilePath, public FileChooser : FileChooser, public toast : ToastController, public plt : Platform, public Calendar : Calendar, public navCtrl : NavController, public params : NavParams, public userService : UserService, public alertCtrl : AlertController, public loadingCtrl : LoadingController, public modalCtrl : ModalController, public actionSheetCtrl : ActionSheetController, public helper : Helper,) {
  ...
  listenLoad() {
    ...
    this.subForground = this
      .helper
      .emitterNotifFourground
      .subscribe((res : any) => {
  ...
  listenNotifFourground(notification : any) {
    let toast = this
      .toast
      .create({message: notification.title, duration: 3500, position: 'top'});
    toast.present();
  }
  ...
  listenNotification(data : any) {
    ...
    // let toast = this.toast.create({   message: data.title,   duration: 3500,
    // position: 'top' }); toast.present();
    ...
    if (data.additionalData && (data.additionalData.typeNotif == 'order' || data.additionalData.typeNotif == 'finish')) {
      ...
      dataOrderDetail.viewReview = data.additionalData.typeNotif == 'finish'
        ? true
        : false;
      ...
      return this
        .navCtrl
        .push(OrderDetailPage, dataOrderDetail);
    }
    ...
    if (data.additionalData && data.additionalData.typeNotif == 'chat') {
      if (data.additionalData.type == 'user') {
        ...
        if (this.navCtrl.getActive().component.name == 'KonsultasiPage') 
          return this.helper.emitterChatTutor.emit(dataNotif);
        return this
          .navCtrl
          .push(KonsultasiPage, dataNotif);
      }
      if (data.additionalData.type == 'cs') {
        if (this.navCtrl.getActive().component.name == 'CustomerservicePage') 
          return undefined;
        return this
          .navCtrl
          .push(CustomerservicePage, {
            data: firebase
              .auth()
              .currentUser
              .uid,
    ...
    this
      ...
      .registerBackButtonAction(() => {
        ...
        if (this.count <= 1) {
          let toaster = this
            .toast
            .create({message: 'press back once more to exit the app', duration: 2000, position: 'bottom'});
          toaster.present();
        }
        if (this.count > 1) {
          this
  ...
  ionViewWillEnter() {
    ...
    let disconnectSubscription = this
      .network
      .onDisconnect()
      .subscribe(() => {
  ...
  getUser() {
    let loader = this
      .loadingCtrl
      .create({content: '<img src="./assets/loading.gif"/>', spinner: 'hide'});
    loader.present();
    setTimeout(() => {
      this.myUserId = firebase
        .auth()
        .currentUser
        .uid;
      ...
      this
        ...
        .subscribe(snapshot => {
          ...
          if (data.smsVerificationStatus != true) {
            ...
            this
              .userService
              .sendVerificationCode(this.user)
              .subscribe(res => {
                if (res.result) {
                  alert.setMessage('<span style="color:green !important;">Kode verifikasi Anda telah terkirim ke nom' +
                      'or ' + data.phoneNumber + '</span>');
                  return false;
                } else {
                  alert.setMessage('<span style="color:green !important;">Kode verifikasi Anda gagal dikirim ke nomo' +
                      'r ' + data.phoneNumber + ' tolong kontak CS utk kode verifikasi</span>');
                  return false;
                }
              });
          } else {
  ...
  chatNotificationCs() {
    this.csSubscribe = this
      .userService
      .getChatCsStatus(this.myUserId)
      .subscribe(snapshot => {
        this.chatCs = 0;
        let data = [];
        if (snapshot.length) {
  ...
  chatNotificationMurid() {
    this.tutorSubscribe = this
      .userService
      .getChatTutorStatus(this.myUserId)
      .subscribe(snapshot => {
        this.chatData = 0;
        let data = [];
        if (snapshot.length) {
          snapshot.forEach(o => {
            ...
            // o.messages.forEach(m => {   if (m.status == false && m.position == 'left') {
            //   data.push(m);   } })
          })
        }
  ...
  test() {
    this
      .zapier
      .getUnavailability("2017-07-04", "UEE50q8PMnVU0H8bUiUSIn7D6762")
      .take(1)
      .subscribe(res => {
        console.log(res)
        alert(res.status);
  ...
  navigateMe(name : string) {
    ...
    if (name == 'KategoriPage') 
      this.navCtrl.push(KategoriPage, {
        uid: this.myUserId,
        source: 'home'
      });
    if (name == 'CustomerservicePage') 
      this.navCtrl.push(CustomerservicePage, {
        data: this.myUserId,
        name: this.user.firstName
      });
    if (name == 'KonsultasiPage') 
      this.navCtrl.push(KonsultasiPage, {
        uid: this.myUserId,
        name: this.user.firstName
      });
    ...
    if (name == 'SettingPage') 
      this.navCtrl.push(SettingPage, {uid: this.myUserId});
    }
  ...
  initDataCalendar(openCalendar
    ...
    this
      .userService
      .getCart(firebase.auth().currentUser.uid)
      .once('value', (res : any) => {
        let val = res.val();
        if (!val) {
          this
  ...
  async cekSchedule(allOrder : any[], openCalendar
    ...
    for (let i = 0; i < allOrder.length; i++) {
      if (allOrder[i].status == 'booked') {
        ...
        let alamat = allOrder[i].alamatMurid
          ? allOrder[i].alamatMurid
          : 'Rumah Murid';
        let schedule = allOrder[i]
          .sessions
    ...
    for (let i = 0; i < list.length; i++) {
      await this
        .Calendar
        .findEventWithOptions(list[i].title, list[i].location, list[i].note, list[i].startDate, list[i].endDate, options)
        .then((res : any) => {
  ...
  async setScheduleData(list : any, options) {
    await this
      .Calendar
      .createEventWithOptions(list.title, list.location, list.note, list.startDate, list.endDate, options)
      .then()
  }
  ...
  private presentToast(text) {
    let toast = this
      .toast
      .create({message: text, duration: 3000, position: 'top'});
    toast.present();
  }
  ...
  // resendSMS(smsObj) {   this.zapier.sendVerificationCode(smsObj).subscribe(res
  // => {     console.log(res);     if (res['_body'] == 'success') {       return
  ...
  updateUserStatus(uid) {
    this
      .userService
      .updateUserData(uid, {smsVerificationStatus: true})
      .then(() => {
        this.status = true;
        return true
      })
